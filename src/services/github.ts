...
import connectDb, { db } from '../db/db'; // ✅ fixed import
...
export const getOrgsUnassignedIssues = async (orgs: string[]): Promise<any[]> => { 
    try {
        ...
        for (const org of orgs) {
            ...
            for (const repo of topRepos) {
                ...
                for (const issue of issues) {
                    await db.collection('lfx_issues').updateOne( // ✅ works now
                        { id: issue.id },
                        { $set: issue },
                        { upsert: true }
                    );
