...
import { db } from '../../db/db'; // ✅ correct import
...
export const getUnassignedIssues = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const repoWithMostOpenIssues = repos.reduce(
            (prev: { open_issues: number }, current: { open_issues: number }) =>
                prev.open_issues > current.open_issues ? prev : current
        );
...
export const getPopularIssues = async (req: Request, res: Response): Promise<void> => {
    try {
        // ✅ explicitly type query params as strings
        const label = typeof req.query.label === "string" ? req.query.label : "";
        const organizations = typeof req.query.organizations === "string" || Array.isArray(req.query.organizations) 
